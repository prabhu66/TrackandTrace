...
@Component({
  selector: 'app-geofence-update',
  templateUrl: './geofence-update.component.html',
  styleUrls: ['./geofence-update.component.css']
})
export class GeofenceUpdateComponent implements AfterViewInit {
  ...
  public pageTitle: string = "Map";
   ...
   constructor(private _location: Location, private _assetService:AssetService,private _dataService: MapService,private router: Router, private matIconRegistry: MatIconRegistry, private domSanitizer: DomSanitizer,private snackBar: MatSnackBar) { 
      ...
      if(this.status==true){
        ...
        if(this.Coordinates.length==0){
          ...
          let newObject={
           ...
           this._dataService.geofence=JSON.parse(res).res;
          });
          jQuery('#geofenceEdit').modal('hide');
        }
